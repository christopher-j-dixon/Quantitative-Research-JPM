...
for feature in ['total_debt_outstanding', 'loan_amt_outstanding']:
    df[feature], _ = boxcox(features_df[feature] + 1)
...
log_model = LogisticRegression(C=0.01, solver='liblinear').fit(X_train_normalized, y_train)
y_pred_log = log_model.predict(X_test_normalized)

# Logistic Regression Model testing
print('Jaccard Score (Logistic Regression) =', jaccard_score(y_test, y_pred_log, pos_label=0))
...
# Random Forest Model
rf_model = RandomForestClassifier(n_estimators=100, random_state=42).fit(X_train_normalized, y_train)
